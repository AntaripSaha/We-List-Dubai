<template>
  <div class='relative pb-32 bg-accent-color-50'>
    <div class='flex justify-between items-center px-4 pt-6 pb-16 mx-auto max-w-7xl sm:px-6 lg:px-8'>
      <div class='flex-1 min-w-0'>
        <h2 class='py-3 text-xl font-bold lg:text-3xl text-second-color-600'>Popular in Real Estate Listings</h2>
  ...
  <section aria-labelledby='contact-heading'
    ...
    <div class='grid grid-cols-2 gap-y-3 gap-x-3 lg:grid-cols-5 lg:gap-y-3 lg:gap-x-3'>
      <EServiceItem v-for='eService in realestateEServices' :key='eService.id' :eService='eService' />
    </div>
  </section>
</template>
...
export default {
  ...
  mounted() {
    this.getRealEstateEServicesAction()
  },
  computed: {
    ...mapState(['realestateEServices']),
  },
  methods: {
    ...mapActions(['getRealEstateEServicesAction']),
  },
}
</script>
