  ...
  <section>
    ...
    <div v-else class='rounded-lg bg-white overflow-hidden shadow-lg p-8'>
      ...
      <ul class='grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8' role='list'>
        ...
        <photo-provider>
          <photo-consumer v-for="image in eProviderGalleries" :intro="this.$filters.stripHtml(image.description)" :key="image.id" :src="this.$filters.getFirstMediaUrl(image)">
            <img :src="this.$filters.getFirstMediaThumb(image)" class="w-full custom-img-height aspect-w aspect-h-8 rounded-lg object-cover">
          </photo-consumer>
      </photo-provider>
      </ul>
